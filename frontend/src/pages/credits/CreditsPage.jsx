...
const FILTER_FIELDS_BASE = [
  { key: "search",  label: "Recherche", type: "text",   placeholder: "N° crédit, client…" },
  { key: "statut",  label: "Statut",    type: "select",
    options: Object.entries(STATUT_META).map(([v, m]) => ({ value: v, label: m.label })),
  },
...
const COLUMNS = [
  {
    key: "numero_credit", label: "N° Crédit", width: "16%",
    render: (val, row) => (
  ...
  {
    key: "montant_total", label: "Total", width: "13%",
    render: (val) => <span className={styles.montantText}>{fmt(val)}</span>,
  },
  {
    key: "reste", label: "Reste", width: "13%",
    render: (val) => (
...
export default function CreditsPage() {
  const {
    credits, pagination, total, stats, numeroPreview,
    fetchLoading, createLoading, updateLoading, deleteLoading, paiementLoading,
    fetchCredits, createCredit, updateCredit, deleteCredit,
    enregistrerPaiement, generateNumeroCredit,
  } = useCredits(); 
  
  const { activeClients, fetchActiveClients } = useClients();
  useEffect(() => { fetchActiveClients(); }, []); 
  ...
  // ✅ filterFields calculé dans le composant avec les clients chargés dynamiquement
  const filterFields = [
    ...FILTER_FIELDS_BASE,
    {
  ...
  const tableActions = [
    ...
    { key: "delete", label: "Supprimer", icon: <TrashIcon />, onClick: setDeleteConfirm    },
  ];
