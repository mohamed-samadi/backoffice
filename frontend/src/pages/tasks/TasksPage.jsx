...
const FILTER_FIELDS = [
  ...
  { key: "due_date_to", label: "Au", type: "datee", placeholder: "AAAA-MM-JJ" },
  {
    key: "sort_by",
    label: "Trier par",
    type: "select",
...
const COLUMNS = [
        ...
        <div>
          ...
          {row.category?.name && (
            <div
              ...
              style={{
                color: row.category?.color || "var(--color-text-muted)",
              }}
            >
              {row.category.color && (
                <span
  ...
  },
  {
    ...
    render: (val) => {
      ...
      return (
        <span
          className={`${styles.statusBadge} ${styles[`status--${meta.color}`]}`}
        >
  ...
  },
  {
    ...
    render: (val, row) => {
      ...
      const isOverdue =
        new Date(val) < new Date() && row.status !== "completed";
      return (
