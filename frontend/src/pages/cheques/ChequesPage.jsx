...
const FILTER_FIELDS_BASE = [
  {
    ...
    label: "Recherche",
    type: "text",
    placeholder: "N° chèque, banque, titulaire…",
  },
  {
    key: "statut",
    label: "Statut",
...
const COLUMNS = [
    ...
    },
  {
    ...
    render: (val, row) => (
      <div className={styles.cellCheque}>
        <div
          className={`${styles.statutBar} ${styles[`bar--${STATUT_META[row.statut]?.color}`]}`}
        />
        <div>
  ...
  },
  {
    ...
    render: (val, row) => {
      ...
      const isOverdue =
        new Date(val) < new Date() && row.statut === "non_encaisse";
      return (
...
export default function ChequesPage() {
  ...
  const filterFields = [
    ...
    { key: "banque", label: "Banque", type: "select", options: banques.map((c) => ({
        value:  c,
        label: c,
  ...
  const handleSubmit = async (formData) => {
    try {
      if (modal.mode === "create") {
        ...
        await fetchCheques({ ...filters, page: 1 }); // reload page 1 to show new cheque
  ...
  const tableActions = [
    ...
    { key: "edit", label: "Modifier", icon: <EditIcon />, onClick: openEdit },
    {
      ...
      icon: <CheckIcon />,
      onClick: (row) => {
        if (row.statut === "non_encaisse")
          setActionConfirm({ type: "encaisser", cheque: row });
    ...
    },
    {
      ...
      icon: <AlertIcon />,
      onClick: (row) => {
        if (["non_encaisse", "encaisse"].includes(row.statut))
          setActionConfirm({ type: "impaye", cheque: row });
    ...
    },
    {
      ...
      icon: <XIcon />,
      onClick: (row) => {
        if (row.statut !== "encaisse")
          setActionConfirm({ type: "annuler", cheque: row });
        else
          notify(
            "error",
  ...
  const actionMeta = {
    encaisser: {
      label: "Encaisser",
      btnClass: styles.actionBtnGreen,
      msg: "Confirmer l'encaissement de ce chèque ?",
    },
    impaye: {
      label: "Marquer impayé",
      btnClass: styles.actionBtnRed,
      msg: "Marquer ce chèque comme impayé ?",
    },
    annuler: {
      label: "Annuler",
      btnClass: styles.actionBtnAmber,
      msg: "Confirmer l'annulation de ce chèque ?",
    },
  };
      ...
      {actionConfirm && (
        <div className={styles.backdropDelete}>
          <div className={styles.deleteCard}>
            <div
              className={`${styles.deleteIcon} ${styles[`deleteIcon--${actionConfirm.type}`]}`}
            >
              {actionConfirm.type === "encaisser" ? (
                <CheckIcon />
              ) : actionConfirm.type === "impaye" ? (
                <AlertIcon />
              ) : (
                <XIcon />
              )}
            ...
            <p className={styles.deleteMsg}>
              ...
              {" — "}
              {Number(actionConfirm.cheque.montant).toLocaleString("fr-FR", {
                style: "currency",
                currency: "MAD",
              })}
            </p>
