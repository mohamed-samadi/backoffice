...
export default function ProductModal({
  ...
  const title  = mode === "create" ? "Nouveau produit"
  ...
  useEffect(() => {
    if (initialData) {
      setForm({
        ...
        fournisseur_id: initialData.fournisseur?.id ?? initialData.fournisseur_id ?? "",
  ...
  const handleImageFile = useCallback((file) => {
    ...
    if (!file.type.startsWith("image/")) {
      setErrors((prev) => ({ ...prev, image: "Fichier image requis (jpg, png, webp…)" }));
      return;
    }
    if (file.size > 2 * 1024 * 1024) {
      setErrors((prev) => ({ ...prev, image: "Image trop lourde (max 2 Mo)" }));
      return;
    }
    if (imagePreview?.startsWith("blob:")) URL.revokeObjectURL(imagePreview);
    setImageFile(file);
  ...
  const validate = () => {
    ...
    if (Number(form.prix_unitaire_ht) < 0)                    errs.prix_unitaire_ht = "Prix invalide";
    if (form.tva === "" || form.tva === null)                  errs.tva              = "La TVA est obligatoire";
    if (Number(form.tva) < 0 || Number(form.tva) > 100)       errs.tva              = "TVA entre 0 et 100%";
    if (form.prix_revient === "" || form.prix_revient === null)errs.prix_revient     = "Le prix de revient est obligatoire";
    if (Number(form.prix_revient) < 0)                        errs.prix_revient     = "Prix de revient invalide";
    if (form.quantite_stock === "" || form.quantite_stock === null)
                                                              errs.quantite_stock   = "Le stock est obligatoire";
    if(Number(form.prix_revient) > 0 && Number(form.prix_unitaire_ht) > Number(form.prix_revient))
      errs.prix_revient = "Le prix de revient doit être supérieur ou égal au prix ht";
        ...
        <div className={styles.body}>
            ...
            <div className={styles.fieldGroup}>
              ...
              {imagePreview ? (
                ...
                >
                  ...
                  <span className={styles.dropzoneHint}>JPG, PNG, WEBP — max 2 Mo</span>
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*"
          ...
          <div className={styles.grid2}>
            ...
            <div className={`${styles.fieldGroup} ${styles.colSpan2}`}>
              ...
              {isView ? (
                <ViewValue><span className={styles.viewText}>{form.nom}</span></ViewValue>
              ) : (
                <>
                  <input
                    id="nom"
            ...
            <div className={`${styles.fieldGroup} ${styles.colSpan2}`}>
              <label className={styles.label} htmlFor="code_barre">Code-barres</label>
              {isView ? (
                <ViewValue>
                  {form.code_barre
          ...
          <div className={styles.grid2}>
            ...
            {/* Prix HT */}
            <div className={styles.fieldGroup}>
              <label className={styles.label} htmlFor="prix">Prix unitaire HT *</label>
              {isView ? (
                <ViewValue>
                  {form.prix_unitaire_ht != null
                    ? Number(form.prix_unitaire_ht).toLocaleString("fr-FR", { style: "currency", currency: "MAD" })
                    : null}
                </ViewValue>
              ) : (
                <>
                  <div className={styles.inputWithAddon}>
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      onChange={(e) => handleChange("prix_unitaire_ht", e.target.value)}
                    />
                  </div>
                  {errors.prix_unitaire_ht && <span className={styles.errorMsg}>{errors.prix_unitaire_ht}</span>}
                </>
              )}
            </div>
            ...
            {/* Prix de revient */}
            <div className={styles.fieldGroup}>
              <label className={styles.label} htmlFor="prix_revient">Prix de revient *</label>
              {isView ? (
                <ViewValue>
                  {form.prix_revient
                    ? Number(form.prix_revient).toLocaleString("fr-FR", { style: "currency", currency: "MAD" })
                    : null}
                </ViewValue>
              ) : (
                <>
                  <div className={styles.inputWithAddon}>
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      onChange={(e) => handleChange("prix_revient", e.target.value)}
                    />
                  </div>
                  {errors.prix_revient && <span className={styles.errorMsg}>{errors.prix_revient}</span>}
                </>
              )}
            </div>
            ...
            {/* TVA */}
            <div className={styles.fieldGroup}>
              <label className={styles.label} htmlFor="tva">TVA *</label>
              {isView ? (
                <ViewValue>
                ...
                <>
                  ...
                  {errors.tva && <span className={styles.errorMsg}>{errors.tva}</span>}
                </>
              )}
            </div>
            ...
            {(isView || (form.prix_unitaire_ht && form.tva)) && (
              <div className={styles.fieldGroup}>
                ...
                <div className={`${styles.viewValue} ${styles.viewValueAccent}`}>
                  {Number(
            ...
            {/* Stock */}
            <div className={styles.fieldGroup}>
              <label className={styles.label} htmlFor="stock">Quantité en stock *</label>
              {isView ? (
                <ViewValue>
                  <span className={
                    (form.quantite_stock ?? 0) < 10 ? styles.stockLow
                    : (form.quantite_stock ?? 0) < 50 ? styles.stockMid
                    : styles.stockOk
                  }>
                    {form.quantite_stock ?? 0} unité{(form.quantite_stock ?? 0) !== 1 ? "s" : ""}
                  </span>
                </ViewValue>
              ) : (
                <>
                  <input
                    ...
                    placeholder="0"
                    onChange={(e) => handleChange("quantite_stock", e.target.value)}
                  />
                  {errors.quantite_stock && <span className={styles.errorMsg}>{errors.quantite_stock}</span>}
                </>
              )}
            </div>
            ...
            {/* Seuil alerte stock */}
            <div className={styles.fieldGroup}>
              <label className={styles.label} htmlFor="seuil">Seuil alerte stock</label>
              {isView ? (
                <ViewValue>
                  {form.seuil_alerte_stock ?? 10} unité{(form.seuil_alerte_stock ?? 10) !== 1 ? "s" : ""}
                </ViewValue>
              ) : (
                <input
                  id="seuil"
                  ...
                  placeholder="10"
                  onChange={(e) => handleChange("seuil_alerte_stock", e.target.value)}
                />
              )}
            </div>
              ...
              {isView ? (
                <ViewValue>
                  {form.fournisseur?.nom
                    || fournisseurs.find((f) => f.id === Number(form.fournisseur_id))?.nom
                    || null}
                </ViewValue>
              ) : (
                <select
