...
export default function ClientModal({
  ...
  const title  = mode === "create" ? "Nouveau client"
               : mode === "edit"   ? "Modifier le client"
               :                     "Détails du client";
      ...
      <div className={styles.modal}>
        ...
        <div className={styles.body}>
          ...
          <div className={styles.avatarSection}>
            <div className={`${styles.avatar} ${form.statut === "active" ? styles.avatarActive : styles.avatarInactive}`}>
              {form.nom_complet?.charAt(0)?.toUpperCase() || <UserIcon />}
            </div>
            <div className={styles.avatarInfo}>
              <span className={styles.avatarName}>
                {form.nom_complet || <span className={styles.viewEmpty}>Nom du client</span>}
              </span>
              {form.nom_entreprise && (
                <span className={styles.avatarEntreprise}>{form.nom_entreprise}</span>
              )}
            </div>
            {/* Toggle statut */}
            {!isView && (
              <button
                ...
                onClick={() => handleChange("statut", form.statut === "active" ? "inactive" : "active")}
              >
            ...
            {isView && (
              <span className={form.statut === "active" ? styles.badgeActive : styles.badgeInactive}>
          ...
          <div className={styles.grid2}>
            ...
            <div className={`${styles.fieldGroup} ${styles.colSpan2}`}>
              <label className={styles.label} htmlFor="nom_complet">Nom complet *</label>
              {isView ? (
                <ViewValue><span className={styles.viewText}>{form.nom_complet}</span></ViewValue>
              ) : (
                <>
                  <input
                  ...
                  />
                  {errors.nom_complet && <span className={styles.errorMsg}>{errors.nom_complet}</span>}
                </>
              )}
            </div>
            ...
            {/* Entreprise */}
            <div className={`${styles.fieldGroup} ${styles.colSpan2}`}>
              <label className={styles.label} htmlFor="nom_entreprise">Entreprise</label>
              {isView ? (
          ...
          <div className={styles.grid2}>
            ...
            <div className={styles.fieldGroup}>
              ...
              {isView ? (
                <ViewValue>
                  {form.telephone
                    ? <a href={`tel:${form.telephone}`} className={styles.contactLink}>{form.telephone}</a>
                    : null}
                </ViewValue>
              ) : (
            ...
            <div className={styles.fieldGroup}>
              ...
              {isView ? (
                <ViewValue>
                  {form.email
                    ? <a href={`mailto:${form.email}`} className={styles.contactLink}>{form.email}</a>
                    : null}
                </ViewValue>
              ) : (
                <>
          ...
          <div className={styles.fieldGroup}>
            <label className={styles.label} htmlFor="adresse">Adresse complète</label>
            {isView ? (
                ...
                <div className={styles.fieldGroup}>
                  ...
                  <ViewValue>
                    {initialData.updated_at
                      ? new Date(initialData.updated_at).toLocaleDateString("fr-FR", {
                          day: "2-digit", month: "long", year: "numeric",
                        })
                      : null}
                  </ViewValue>
