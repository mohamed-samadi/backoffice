...
const TYPE_META = {
  facture: {
    label: "Facture",
    secondaryDateLabel: "Echeance",
    secondaryDateField: "date_echeance",
    showAmounts: true,
    showPayments: true,
    signatureLabel: "Signature & Cachet",
    linesTitle: "Detail des prestations",
  },
  devis: {
    label: "Devis",
    secondaryDateLabel: "Validite",
    secondaryDateField: "date_validite",
    showAmounts: true,
    showPayments: false,
    signatureLabel: "Bon pour accord",
    linesTitle: "Detail de l'offre",
  },
  bon_livraison: {
    ...
    showAmounts: false,
    showPayments: false,
    signatureLabel: "Reception client",
    linesTitle: "Articles livres",
  },
};

const getTypeMeta = (type) => TYPE_META[type] || TYPE_META.facture;
...
/* ─── PDF HTML builder ───────────────────────────────────────────────────── */
// Shared with DocumentDetailPage for the WhatsApp PDF print flow.
// eslint-disable-next-line react-refresh/only-export-components
export function buildPdfHtml(doc, companyInfo = {}) {
  ...
  const secondaryDate = doc[meta.secondaryDateField] || doc.date_validite;
    ...
    ? lines.map((l, i) => {
        ...
        return meta.showAmounts
          ...
          : `<tr>
            ...
            <td>${l.description || l.product?.nom || `Produit ${i + 1}`}</td>
            <td class="center">${qty}</td>
            <td>${l.product?.sku || l.reference || ""}</td>
          </tr>`;
      }).join("")
    : `<tr><td colspan="${meta.showAmounts ? 6 : 4}" class="muted center">Aucune ligne</td></tr>`;
...
<body>
  <div class="header">
    ...
    <div>
      ...
      <div class="doc-ref">
        ...
        ${meta.secondaryDateLabel} : ${fmtDate(secondaryDate)}
      </div>
    </div>
  </div>
  ...
  <p class="section-title">${meta.linesTitle}</p>
  <table>
    <thead>
      ${meta.showAmounts
        ? `<tr>
          <th style="width:36%">Description</th>
          <th class="center" style="width:7%">QTE</th>
        ...
        : `<tr>
          <th style="width:8%">#</th>
          <th>Description</th>
          <th class="center" style="width:14%">QTE</th>
  ...
  ${meta.showAmounts ? `<div class="totals-wrap">
    <div class="totals">
  ...
  ${meta.showPayments && payments.length ? `<div style="margin-bottom:18px">
  ...
  ${co.conditions ? `<div class="conditions">
    <p class="section-title">Conditions</p>
    <p>${co.conditions}</p>
  </div>` : ""}

  <div class="signature-row">
    <div class="signature-box"><span class="signature-label">${meta.signatureLabel}</span></div>
  </div>
...
function PrintPreviewModal({ doc, companyInfo, onClose }) {
  ...
  /* Native browser PDF flow. In the print dialog, choose "Save as PDF". */
  const handleDownloadPdf = async () => {
    setStatus("loading");
    setErrMsg("");
    try {
      const fw = iframeRef.current?.contentWindow;
      if (!fw) throw new Error("Preview is not ready");
      fw.focus();
      fw.print();
      setStatus("idle");
    } catch (err) {
          ...
          <div className={styles.controlBody}>
            ...
            <div className={styles.docSummary}>
              ...
              <div className={styles.summaryMeta}>
                ...
                {secondaryDate ? ` · ${meta.secondaryDateLabel} ${fmtDate(secondaryDate)}` : ""}
              </div>
              <div className={styles.summaryTotal}>
                {meta.showAmounts ? currency(doc.total_ttc) : `${(doc.documentLines || doc.document_lines || []).length} ligne(s)`}
              </div>
            </div>
