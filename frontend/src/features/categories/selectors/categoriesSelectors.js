...
export const selectActiveWithCount = (state) =>
  state?.categories?.activeWithCount || [];

// ─── Stats (retournées par index()) ──────────────────────────────────────────
export const selectCategoriesStats = (state) =>
  state?.categories?.stats || { total: 0, actifs: 0, inactifs: 0 };
...
export const selectCategoriesTotal = (state) => {
  ...
  return metaTotal !== undefined
    ? metaTotal
    : selectCategoriesData(state).length;
};

// ─── Loading granulaire ───────────────────────────────────────────────────────
export const selectCategoriesLoadingStates = (state) =>
  state?.categories?.loadingStates || {};
export const selectCategoryFetchLoading = (state) =>
  state?.categories?.loadingStates?.fetch || false;
export const selectCategoryFetchOneLoading = (state) =>
  state?.categories?.loadingStates?.fetchOne || false;
export const selectCategoryCreateLoading = (state) =>
  state?.categories?.loadingStates?.create || false;
export const selectCategoryUpdateLoading = (state) =>
  state?.categories?.loadingStates?.update || false;
export const selectCategoryDeleteLoading = (state) =>
  state?.categories?.loadingStates?.delete || false;
export const selectCategoryFetchActiveLoading = (state) =>
  state?.categories?.loadingStates?.fetchActive || false;
export const selectCategoryFetchWithCountLoading = (state) =>
  state?.categories?.loadingStates?.fetchWithCount || false;
export const selectCategoryBulkUpdateLoading = (state) =>
  state?.categories?.loadingStates?.bulkUpdate || false;
...
export const selectCategoriesSuccess = (state) =>
